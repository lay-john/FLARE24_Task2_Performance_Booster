...
def get_bbox_from_mask_b(nonzero_mask, outside_value):
    # 找到非outside_value的坐标
    mask_voxel_coords = torch.nonzero(nonzero_mask != outside_value, as_tuple=True)

    if len(mask_voxel_coords[0]) == 0:
        return None

    # 计算每个维度的最小和最大索引
    minzidx = int(torch.min(mask_voxel_coords[0]).item())
    maxzidx = int(torch.max(mask_voxel_coords[0]).item()) + 1
    minxidx = int(torch.min(mask_voxel_coords[1]).item())
    maxxidx = int(torch.max(mask_voxel_coords[1]).item()) + 1
    minyidx = int(torch.min(mask_voxel_coords[2]).item())
    maxyidx = int(torch.max(mask_voxel_coords[2]).item()) + 1
...
class SegmentationNetwork(NeuralNetwork):
    ...
    @staticmethod
    def _compute_steps_for_sliding_window_22(patch_size: Tuple[int, ...], image_size: Tuple[int, ...], step_size: float) -> List[List[int]]:
        assert [i >= j for i, j in zip(image_size, patch_size)], "image size must be as large or larger than patch_size"
        assert 0 < step_size <= 1, 'step_size must be larger than 0 and smaller or equal to 1'
        ...
        if image_size[1] > 2.0 * patch_size[1]:
            steps_here = [int(np.round(image_size[1]/2)-1.0*patch_size[1]), int(np.round(image_size[1]/2)-0.5*patch_size[1]), int(np.round(image_size[1]/2)+0.*patch_size[1])]
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        if shape[1] > 15 / 8 * patch_size[1] and shape[1] - 15 / 8 * patch_size[1] < 40:
            x_low = (math.floor(3 / 5 * (shape[1] - 15 / 8 * patch_size[1])))
            x_high = (x_low + 15 / 8 * patch_size[1])
        if shape[2] > 15 / 8 * patch_size[2] and shape[2] - 15 / 8 * patch_size[2] < 30:
            y_low = (math.floor(1 / 2 * (shape[2] - 15 / 8 * patch_size[2])))
            y_high = (y_low + 15 / 8 * patch_size[2])
        
        
        if shape[0] > int(np.ceil(patch_size[0] + 14 / 8 * patch_size[0])):
            ...
            for i in range(dd):
                ...
                # x = data[None, :, int(lb_z): int(ub_z), int(lb_x): int(ub_x), int(lb_y): int(ub_y)]
                # x = x.numpy()
                predicted_patch = self._internal_maybe_mirror_and_pred_3D(
                    data[None, :, lb_z:ub_z, lb_x:ub_x, lb_y:ub_y], mirror_axes, do_mirroring)[0]
                pred_label = torch.argmax(predicted_patch, 0)
                indices = torch.sum(pred_label == 1)
                print("indices!!!!!!!!!!!!!!!!", indices)
                if indices < 10000:  # 确保找到标签为1的区域
                    lb_z += patch_size[0]
                    del indices
                    continue
                else:
                    ...
                    qq = lb_z + box[0][0]
                    print(lb_z)
                    low = int(qq - math.floor(patch_size[0] * 9 / 8)) # 9 / 8
                    high = int(qq + math.floor(patch_size[0] * 20 / 8)) # 17 / 8
                    print(low, high)
                    break
        ...
        print(np.array([high - low, x_high - x_low, y_high - y_low]))
        steps = self._compute_steps_for_sliding_window(patch_size,
                                                       np.array([high - low, x_high - x_low, y_high - y_low]),
                                                       step_size)
        print(steps)
        print(steps[0])
        print(low, high)
        steps[0] = [x + low for x in steps[0]]
        steps[1] = [x + x_low for x in steps[1]]
        steps[2] = [x + y_low for x in steps[2]]
        print("new", steps)
        ###########################################
