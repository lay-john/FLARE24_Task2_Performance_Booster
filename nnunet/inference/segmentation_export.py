...
def save_segmentation_nifti(segmentation, out_fname, dct, order=1, force_separate_z=None, order_z=0, verbose: bool = False):
    ...
    predict_final = np.zeros(shape_original_before_cropping, dtype=np.uint8)
    predict_final[resizer] = seg_old_spacing
    ...
    seg_resized_itk = sitk.GetImageFromArray(predict_final)
